...
//	);
		...
		'webscr' => 'https://www.paypal.com/webscr/',
		'endpoint' => 'https://api-3t.paypal.com/nvp/',
		'password' => 'Y7KYGLDTV9J2N8RU',
		'email' => 'payments_api1.finalearth.com',
		'signature' => 'AphyFFYL-ZFXrWOOVE9mXkbIljV8APZCAwNgxYXDdUHvHbwwT6k4tlBq'
	);
	...
	public $sandboxConfig = array(
		...
		'password' => '1394443374',
		'email' => 'animikaelyan_api1.gmail.com',
		'signature' => 'An5ns1Kso7MWUdW4ErQKJJJ4qi4-Awwqmgk5fHzGJwJgKlmSiPyzgUJb'
	);

	// API version
	public $apiVersion = '76.0';
	...
	public function initialize(Controller $controller) {
		...
		if($this->sandboxMode) {
			$this->config = $this->sandboxConfig;
                    ...
                    $this->_controller->loadModel('Setting');
                    $config = $this->_controller->Setting->find('first', array('conditions' => array('name' => 'paypal')));
                    $config = json_decode($config['Setting']['data'], true);
                    $this->config['webscr'] = $config['webscr'];
...
//                $this->config['email'] = Configure::read('Paypal.Email');
//		$this->config['password'] = Configure::read('Paypal.Password');									
//		$this->config['signature'] = Configure::read('Paypal.Signature');
	}
	...
	public function expressCheckout() {
		...
		if(!array_key_exists('TOKEN' , $token)) {
			throw new Exception(__('There was a problem with the payment gateway'));
		}            
	...
	public function doExpressCheckoutPayment() {
        ...
        public function createRecurringPaymentProfile($date = null ,$period = null ,$billingCycle = null) {
                        ...
                        $createRecurringPaymentProfile = array(
		...
		$httpSocket = new HttpSocket();	
		// Post the NVPs to the relevent endpoint
		$response = $httpSocket->post($this->config['endpoint'], $createRecurringPaymentProfile);

		// Handle the response
		return $this->handleResponse($response);

	}
        public function manageRecurringPaymentProfile($profileId = null) {
                $manageRecurringPaymentProfile = array(
		...
		$httpSocket = new HttpSocket();	
		// Post the NVPs to the relevent endpoint
		$response = $httpSocket->post($this->config['endpoint'], $manageRecurringPaymentProfile);
