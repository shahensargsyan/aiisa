<h3 class="text-center">Modify Page</h3>
<hr class="separator">
<div class=" margauto">
    <?php
        if(isset($error_msg)){
    ...
    ?>
    <?php
        echo $this->Form->Create('Footer', array('class' => 'form-horizontal'));
        if(!isset($page)){?>
    ...
    <?php if(!isset($duplicate_id)){?>
        <div class="control-group navFieldset">
            ...
            <div class="controls">
            <?php
                $options = array('top' => '<span>Top</span>', 'bottom' => '<span>Bottom</span>','publication' => '<span>Publication</span>','submenu' => '<span>submenu</span>','not'=>'<span>Do not publish</span>');
                echo $this->Form->radio('navigation',$options, array('label'=>FALSE));
            ?>
            </div>
        </div>
    <?php }
    ...
    <div class="control-group">
        ...
        <div class="controls">
       <?php echo $this->Form->input('text',array('type'=>'textarea','label'=>FALSE));?>
        </div>
    </div>
    <?php if(isset($duplicate_id)){?>
        <input type="hidden" value="<?php echo $duplicate_id;?>" name="duplicate_id">
    <?php }?>
    <?php if(isset($page)){?>
        <input type="hidden" value="<?php echo true;?>" name="help">
    <?php }?>     
    <div class="controls btnSection">
       <?php echo $this->Form->input('Save',array('type'=>'submit','label'=>FALSE ,'class'=>'btn btn-success', 'div' => false));?>
...
$(document).ready(function(){
    tinyMCE.init({
            ...
            //advlink,,advlist,autoresize,autosave
            //bbcode
            // Theme options
            theme_advanced_buttons1 : "autoresize,fullpage,save,link,newdocument,|,bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull | hr,removeformat,|,sub,sup,|,iespell,advhr,|,print,|,ltr,rtl,|,fullscreen |,image,code",
