...
<table id="rounded-corner" summary="2007 Major IT Companies' Profit" style="margin-top:10px;">
	...
	<?php if(!empty($mailList)):
		  foreach($mailList as $mail):
			$date = explode(" ",$mail['Contact']['submitdate']);
			$date = explode("-",$date[0]);
			$receiveDate = $date[2]."/".$date[1]."/".$date[0];
			$class = ($mail['Contact']['status'] == 0)?'setBold':'';?>
		<tr>
        	<td style="vertical-align:top;"><?php echo $this->Form->input("Select_".$mail['Contact']['id'],array('label' => false,'type' => 'checkbox','class' => 'check','value' => $mail['Contact']['id']));?></td>
            <td style="vertical-align:top;" class=<?php echo $class;?>><?php echo $this->Html->link($mail['Contact']['name'],'viewMail/'.$mail['Contact']['id'].'',array('style' => 'text-decoration:none;'));?></td>
            <td><?php echo $this->Html->link($mail['Contact']['message'],'viewMail/'.$mail['Contact']['id'].'',array('style' => 'text-decoration:none;'));?></td>
